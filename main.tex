...
\input{pieces/form02_2019_header} % pieces
...
ここでは上述した2つの研究目的に関してそれぞれの目的ごとにわけて説明する.
...
現状ストカスティック形式における運動方程式は厳密に決定されないことを指摘した~\cite{Pinol:2018euk}.
...
スカラー場に関する摂動展開を必要としない新たな曲率ゆらぎ計算手法を提唱した (ストカスティック-$\delta N$形式と呼ぶ)~\cite{Fujita:2013cna}.
...
そこでまずは様々な模型に対してストカスティック形式と場の量子論の計算を比較し, この構築法の正しさを検証する.
その後この構築法を再現する, より原理的な UV 積分処方箋を提唱することを目指す.
...
先述した数値コードは中心部分はすでに出来てきているので, コード公開に向けてバグ取りや性能向上, 運用試験などを繰り返していく.
...
PBH は非常に低確率に生成されなければならないので, PDF の裾を計算できる手法は PBH 量の正確な見積もりに重要である.
そこでこのような数値計算コードを実装したのちは様々な PBH 形成模型に適用し, 実際にその有用性を明らかにしていく.
関連して重要なインフレーション模型のクラスが Renaux-Petel 氏らによって提唱された~\cite{Renaux-Petel:2015mga}.
